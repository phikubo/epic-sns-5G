...
class Sistema_Celular:
	...
	def __init__(self, params_simulacion, params_transmision, params_perdidas):
		...
		self.origen_cel_x, self.origen_cel_y=mc.coordenadas_nceldas(self.num_celdas, self.radio, self.debug)
		#----------------------------------------------------------------------
		#SALIDA
		#----------------------------------------------------------------------
		self.cluster=[]
	...
	def inicializar_distribucion(self):
		...
		if self.intensidad != 0:
			if self.distribucion=="ppp":
				self.usuario_x, self.usuario_y=ppp.distribuir_en_celdas(self.radio, self.origen_cel_x,
					self.origen_cel_y, self.intensidad, self.debug)
				...
				if self.mapa_calor[0]: #si true, genera el mapa de calor.
	...
	def inicializar_cluster_usuarios(self):
		...
		if self.debug:
			print("[sistema.inicializar_cluster_usuarios]")
		for celda_unica, su_x, su_y in zip(self.cluster, self.usuario_x, self.usuario_y):
		...
		#print(len(self.cluster),len(self.usuario_x), "********usuarios")
		self.no_usuarios_total=len(self.cluster)*len(self.usuario_x[0]) #todos las celdas=#usuarios
	...
	def inicializar_hiperc_usuarios(self):
		...
		for celda_unica in self.cluster:
			...
			if self.mapa_calor[0]:
				celda_unica.mapa_bandera=self.mapa_calor[0]
	...
	def inicializar_modelo_canal(self):
		...
		params_sim=[self.frequencia_operacion, (self.hiperc_distancias, "m"), self.debug] #distancia siempre en metros.
		...
		if self.mapa_calor[0]:
			...
			params_malla_sim=[self.frequencia_operacion, (self.hiperc_malla_distancias, "m"), self.debug, self.mapa_calor[0]] #distancia siempre en metros.
	...
	def calcular_sinr(self):
		...
		for cnt in range(self.num_celdas): #range numero de celdas
